...
func HLog<T>(_ message:T,
             file:String = #file,
             function:String = #function,
             line:Int = #line) {
    #if DEBUG
    let fileName = URL.init(fileURLWithPath: file).lastPathComponent// (file as NSString).lastPathComponent
    if message is Dictionary<String, Any> {
        print("[\(fileName):line:\(line)]\n\((message as! Dictionary<String, Any>).myDescription(level: 0))")
    } else if message is Array<Any> {
        print("[\(fileName):line:\(line)]\n\((message as! Array<Any> ).myDescription(level: 0))")
