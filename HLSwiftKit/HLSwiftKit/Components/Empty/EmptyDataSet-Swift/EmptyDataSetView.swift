...
public class EmptyDataSetView: UIView {
    ...
    internal func prepareForReuse() {
        ...
        button.layer.borderColor = UIColor(red: 0.80, green: 0.80, blue: 0.80, alpha: 1.00).cgColor
        customView = nil
...
//                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-(padding)-[button(>=0)]-(padding)-|", options: [], metrics: metrics, views: views))
                let newPadding = (width - 120) / 2.0
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-(\(newPadding))-[button(>=0)]-(\(newPadding))-|", options: [], metrics: metrics, views: views))
            } else {
